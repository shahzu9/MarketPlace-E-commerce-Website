  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter products with IDs between 1 and 4
        const filteredProducts = data.filter(
          (product: Product) => product.id >= 1 && product.id <= 4
        );
    ...
    <div className="bg-white min-h-screen px-4 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mx-auto max-w-5xl">
        {featuredProducts.map((product, index) => (
          <div
            key={product.id}
            className="relative w-full max-w-xs mx-auto transition-all duration-300 hover:shadow-xl overflow-hidden"
